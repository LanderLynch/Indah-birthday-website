    ...
    function tick(t) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (p.isBubble) {
          ...
          grad.addColorStop(0.7, `rgba(124,232,240,${p.alpha * 0.35})`);
          grad.addColorStop(1, 'rgba(124,232,240,0)');
          ctx.fillStyle = grad;
          ctx.fill();
          ctx.lineWidth = 0.6;
          ctx.strokeStyle = `rgba(234,247,251,${p.alpha * 0.4})`;
          ctx.stroke();
        } else {
          ctx.arc(x, p.y, p.r, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(124,232,240,${p.alpha})`;
          ctx.fill();
        }
      }
